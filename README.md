...
Three JSON files behind a raw URL; no key needed:

```sh
curl -s https://raw.githubusercontent.com/aidep-dev/aidep-registry/master/registry/openai.json \
  | jq '.[] | select(.api_ids | index("gpt-4-turbo"))'
```

Or one merged, CORS-open URL with a per-id lookup: [aidep.dev/api/registry](https://aidep.dev/api/registry), documented in [aidep.dev/llms.txt](https://aidep.dev/llms.txt). Missing or wrong row: [file it](../../issues/new?template=file-a-row.yml).
...
`verified_at` date it was last confirmed against that page (moved only when the row changes, not every time we look), so you can audit any claim in it without trusting us.
